...
let blockHeight = 10;
let blockColumns = 8; 
let blockRows = 3; //ajoute plus de block
let blockMaxRows = 10; //nombre max de ligne
let blockCount = 0;
...
window.onload = function() {
    ...
    context = board.getContext("2d"); //pour dessiner dans le canvas
    ...
    for (let i = 0; i < blockArray.length; i++) {
        ...
        if (!block.break) {
            if (topCollision(ball, block) || bottomCollision(ball, block)) {
                block.break = true;     // le block casse
                ball.velocityY *= -1;   // le block renvoi la balle
                score += 100;
                blockCount -= 1;
            }
            else if (leftCollision(ball, block) || rightCollision(ball, block)) {
                block.break = true;     // le block casse
                ball.velocityX *= -1;   // renvoi la balle
                score += 100;
                blockCount -= 1;
            }
